...
iam_enabled=$($cds_cli release --json | jq '.[]|select(.current)|.state.variables|.["iam-enabled"]'|tr '"' '#')
...
if [ "$iam_enabled" == "#true#" ];then
    echo "IAM is enabled"
    praxis_url_raw=$($cds_cli release --json | jq '.[]|select(.current)|.state.variables|.["praxis-login-host"]')
    ...
    sed -i -e "s/praxisEndPassword/${praxis_pwd}/g" $iam_file_path
...
if [ -n "$allowed_ip" ]; then
  ...
  test_run+=" -b --mysql.host=$tmp_api"
else
  echo "don't have allowed_ip"
  echo "mysql is"$(svc_ip mysql)
  test_run+=" -b --mysql.host=$(svc_ip mysql)"
fi
echo "====================================="
